    ...
    <div className="grid min-h-screen place-items-center bg-slate-100/80 p-4">
      <form className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-sm" onSubmit={submit}>
        ...
        <button
          type="submit"
          disabled={loading}
          className="mt-5 h-10 w-full rounded-lg bg-slate-900 text-sm font-semibold text-white transition hover:bg-slate-700 disabled:opacity-60"
        >
          {loading ? 'Signing in...' : 'Sign in'}
        </button>

        <p className="mt-4 text-xs text-slate-400">
          Demo: admin@flowforge.app / Admin@123
        </p>
      </form>
    </div>
  );
